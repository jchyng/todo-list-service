...
const TodoItemComponent = memo(function TodoItemComponent({
  ...
  return (
    <div
      ...
      aria-label={`할 일: ${item.title}${item.is_completed ? ' (완료됨)' : ''}${item.is_important ? ', 중요' : ''}`}
      onKeyDown={(e) => {
    ...
    >
      ...
      <Button
        ...
        aria-label={item.is_completed ? `${item.title} 완료 취소` : `${item.title} 완료 처리`}
        type="button"
      >
      ...
      <Button
        ...
        aria-label={item.is_important ? `${item.title} 중요도 제거` : `${item.title} 중요도 설정`}
        type="button"
      >
        <Star
          className={cn(
